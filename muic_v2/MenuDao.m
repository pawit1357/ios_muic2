...
- (NSMutableArray *) getMenu:(ModelMenu *)model
{
    ...
    if (sqlite3_open(dbpath, &db) == SQLITE_OK)
    {
        NSString *querySQL = [NSString stringWithFormat:@"select id,app_id,parent,menu_item,menu_icon,menu_type from tb_menu where status='A' and app_id=%d and parent=-1 order by menu_order asc",model.app_id];
